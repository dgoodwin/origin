...
func TestInvalidProjectEmptyDirQuota(t *testing.T) {
	...
	for _, err := range errs.Errors {
		...
		if err.Field == "volumeConfig.localQuota.fsGroup" {
			emptyDirQuotaError = err
		}
	}
	if emptyDirQuotaError == nil {
		t.Fatalf("expected volumeConfig.localQuota.fsGroup error but got none")
	}
	if emptyDirQuotaError.Type != field.ErrorTypeInvalid {
		t.Errorf("unexpected error for negative volumeConfig.localQuota.fsGroup: %s",
			emptyDirQuotaError.Detail)
	}
}
