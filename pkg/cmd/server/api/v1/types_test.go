...
`,
	...
	for name, test := range tests {
		...
		if test.expected == "" && nodeConfig.VolumeConfig.LocalQuota.FSGroup != nil {
			t.Errorf("Expected empty quota but got: %s",
				*nodeConfig.VolumeConfig.LocalQuota.FSGroup)
		}
		if test.expected != "" {
			if nodeConfig.VolumeConfig.LocalQuota.FSGroup == nil {
				t.Errorf("Expected quota: %s, got: nil", test.expected)
			} else {
				amount := nodeConfig.VolumeConfig.LocalQuota.FSGroup.Amount
